...
@Serializable('data.MarkdownFile')
export abstract class MarkdownFile extends File {
  ...
  toString(): string {
    return `MarkdownFile(${this.name_})`;
  }
  ...
  static newInstance(
      ...
      parentId: string,
      content: string,
      source: Source): MarkdownFile {
    return DataModels.newInstance(
        MarkdownFile,
        getInitMap_(id, name, parentId, FileType.ASSET, source)
